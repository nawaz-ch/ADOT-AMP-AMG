...
# Add Helm repository
helm repo add nawazrepo https://nawaz-ch.github.io/helm-charts
helm repo update
...
echo "--------------------------------------------"
helm upgrade --install catalog nawazrepo/retail-store-sample-catalog-chart \
  --version 2.0.0 \
...
echo "--------------------------------------------"
helm upgrade --install carts nawazrepo/retail-store-sample-cart-chart \
  --version 1.0.0 \
  -f values-cart.yaml \
  --wait \
  --timeout 5m
...
echo "--------------------------------------------"
helm upgrade --install checkout nawazrepo/retail-store-sample-checkout-chart \
  --version 1.0.0 \
...
echo "--------------------------------------------"
helm upgrade --install orders nawazrepo/retail-store-sample-orders-chart \
  --version 2.0.0 \
...
echo "--------------------------------------------"
helm upgrade --install ui nawazrepo/retail-store-sample-ui-chart \
  --version 1.0.0 \
  -f values-ui.yaml \
  --wait \
  --timeout 5m
